...
from database.db_sqlite import save_user
...
class App(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        w = tk.Label(master, text="Nieuwe User", font=bold_font, fg='green')
        ...
        w = tk.Label(master, text="Role:")
        w.grid(row=4, column=0, **options)

        self.role = tk.StringVar()
        w = tk.Entry(master, textvariable=self.role)
        w.grid(row=4, column=1, **options)
        ...
        w = tk.Button(master, text="Voeg toe", command=self.submit)
